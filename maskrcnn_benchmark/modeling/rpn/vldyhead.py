...
class BertEncoderLayer(BertPreTrainedModel):
    ...
    def forward(self, inputs):
        ...
        # voc
        language_dict_features_voc = inputs["voc"]
        hidden_states_voc = language_dict_features_voc["hidden"]
        attention_mask_voc = language_dict_features_voc["masks"]
        
        device_voc = hidden_states_voc.device
        input_shape_voc = hidden_states_voc.size()[:-1]
        extended_attention_mask_voc = self.get_extended_attention_mask(attention_mask_voc, input_shape_voc, device_voc)

        self_attention_outputs_voc = self.attention(
            hidden_states_voc,
            extended_attention_mask_voc,
            None,
            output_attentions=False,
            past_key_value=None,
        )
        attention_output_voc = self_attention_outputs_voc[0]
        outputs_voc = self_attention_outputs_voc[1:]  # add self attentions if we output attention weights
        layer_output_voc = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output_voc
        )
        outputs_voc = (layer_output_voc,) + outputs_voc
        hidden_states_voc = outputs_voc[0]

        language_dict_features_voc["hidden"] = hidden_states_voc

        features_dict = {"visual": inputs["visual"],
                         "lang": language_dict_features,
                         "voc": language_dict_features_voc
                         }
...
class VLFuse(torch.nn.Module):
    ...
    def visual_pyramid_query(self, text_features, visual_features):
    ...
    def forward(self, x):
        ...
        language_dict_features_voc = x["voc"]

        # for idx, visual_feature in enumerate(visual_features):
        #     print(f"visual_feature_{idx}.size():", visual_feature.size())

        # print("language_dict_features['hidden'].size():", language_dict_features['hidden'].size())
        # print("language_dict_features_voc['hidden'].size():", language_dict_features_voc['hidden'].size())
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.TYPE == "MHA-S":
            ...
            if self.use_checkpoint:
                additional_params = {
                    'attention_mask_voc': language_dict_features_voc['masks'],
                    'voc': language_dict_features_voc['hidden']
                }
                q0, q1, q2, q3, q4, l0, l1, l2, l3, l4 = checkpoint.checkpoint(self.b_attn,
                    ...
                    self.dummy_tensor,
                    language_dict_features_voc['masks'],
                    language_dict_features_voc['hidden']
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.SEPARATE_BIDIRECTIONAL and self.cfg.MODEL.DYHEAD.FUSE_CONFIG.DO_LANG_PROJ_OUTSIDE_CHECKPOINT:
                ...
                language_features_voc = l1

            language_dict_features['hidden'] = language_features
            fused_language_dict_features = language_dict_features

            language_dict_features_voc['hidden'] = language_features_voc
            fused_language_dict_features_voc = language_dict_features_voc            
        ...
        features_dict = {"visual": fused_visual_features,
                         ...
                         "voc": fused_language_dict_features_voc}
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE == "bert-base-uncased":
            lang_cfg = BertConfig.from_pretrained("MODEL/bert-base-uncased")
    ...
    def forward(self, x, language_dict_features=None, language_dict_feature_voc=None, embedding_voc=None, embedding=None, swint_feature_c4=None):
        logits = []
        bbox_reg = []
        centerness = []
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None,
                ...
                language_dict_features_voc=None,
                positive_map=None,
                captions=None,
                voc=None,
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_CONTRASTIVE_ALIGN_LOSS:
            # resizer needed
            embedding_voc = language_dict_features_voc['embedded']
            embedding_voc = self.resizer(embedding_voc)
        elif self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
            # no resizer needed
            embedding_voc = language_dict_features_voc['embedded']
        else:
        ...
        if "masks" in language_dict_features_voc:
            text_masks_voc = language_dict_features_voc["masks"]
        else:
            text_masks_voc = None
        
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER:
            embedding_voc = self.tunable_linear.weight[:embedding_voc.size(1), :].unsqueeze(0) + embedding_voc
            language_dict_features_voc['embedded'] = embedding_voc
            language_dict_features_voc['hidden'] = self.tunable_linear.weight[:embedding_voc.size(1), :].unsqueeze(0) + language_dict_features_voc['hidden']
