...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
        ...
        if(box_cls.shape[1] <= 500):#todo
            candidate_inds = box_cls > 1
        else:
            candidate_inds = box_cls > self.pre_nms_thresh
        # candidate_inds = box_cls > self.pre_nms_thresh # full data and isic open
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        # with open("boxlists.pkl", "ab") as f:  # todo
        #     pickle.dump(boxlists, f)
...
def convert_grounding_to_od_logits(logits, box_cls, positive_map, score_agg=None):
    # import pdb;pdb.set_trace()
    scores = torch.zeros(logits.shape[0], logits.shape[1], box_cls.shape[2]+1).to(logits.device) # +1 todo
        ...
        if score_agg == "MEAN":
            ...
            for b_index in range(scores.shape[0]):
                for label_j in positive_map[0]:
                    ...
                    scores[b_index, :, label_j - 1] = logits[b_index, :, torch.LongTensor(positive_map[b_index][label_j])].mean(-1)
        elif score_agg == "MAX":
            # torch.max() returns (values, indices)
            for b_index in range(scores.shape[0]):
                for label_j in positive_map[0]:
                    #对所有bbox，label j 的score = dot product logits 对应类的prompt平均
                    scores[b_index, :, label_j - 1] = logits[b_index, :, torch.LongTensor(positive_map[b_index][label_j])].max(-1)[0]
...
# def convert_grounding_to_od_logits(logits, box_cls, positive_map, score_agg=None):
#     # scores size are batchsize, regbox, label_size?
