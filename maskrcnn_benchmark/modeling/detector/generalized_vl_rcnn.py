...
class GeneralizedVLRCNN(nn.Module):
    ...
    def forward(self, 
        ...
        if self.cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
            language_dict_features_voc, positive_map = self._forward_language_parallel(
                    captions=voc, targets=targets, device=device,
            ...
            if voc is not None:
                #print(voc[0])
                tokenized_voc = self.tokenizer.batch_encode_plus(voc,
                ...
                if self.use_mlm_loss:
                    if not self.mlm_loss_for_only_positives:
                        greenlight_map_voc = None
                    input_ids_voc, mlm_labels_voc = random_word(
                        input_ids=tokenized_voc.input_ids, 
                        mask_token_id=self.tokenizer.mask_token_id,
                        vocabs=self.tokenizer_vocab_ids,
                        padding_token_id=self.tokenizer.pad_token_id,
                        greenlight_map=greenlight_map_voc)
                else:
                    input_ids_voc = tokenized_voc.input_ids
                    mlm_labels_voc = None
                
                
                tokenizer_input_voc = {"input_ids": input_ids_voc,
                                "attention_mask": tokenized_voc.attention_mask}

                if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE:
                    with torch.no_grad():
                        language_dict_features_voc = self.language_backbone(tokenizer_input_voc)
                else:
                    language_dict_features_voc = self.language_backbone(tokenizer_input_voc) # [1,256,768]
                
                # ONE HOT
                if self.cfg.DATASETS.ONE_HOT:
                    new_masks_voc = torch.zeros_like(language_dict_features_voc['masks'],
                                                device=language_dict_features_voc['masks'].device)
                    new_masks_voc[:, :self.cfg.MODEL.DYHEAD.NUM_CLASSES] = 1
                    language_dict_features_voc['masks'] = new_masks_voc

                # MASK ALL SPECIAL TOKENS
                if self.cfg.MODEL.LANGUAGE_BACKBONE.MASK_SPECIAL:
                    language_dict_features_voc["masks"] = 1 - tokenized_voc.special_tokens_mask
                
                language_dict_features_voc["mlm_labels"] = mlm_labels_voc
        ...
        if self.cfg.GLIPKNOW.PARALLEL_LANGUAGE_INPUT:
            ...
            if captions is not None:
                ...
                if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE:
                    ...
                    language_dict_features = self.language_backbone(tokenizer_input) # [1,256,768]
        ...
        if 'vl' in self.cfg.MODEL.SWINT.VERSION:
            ...
            visual_features = self.backbone(images.tensors) # 5个特征层 [1,256,100,136]下采样4次
        ...
        if self.force_boxes:
            ...
            # import pdb;pdb.set_trace() rpnposition
            proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets, language_dict_features, language_dict_features_voc, positive_map,
                                               captions, voc, swint_feature_c4)
        if self.roi_heads:
