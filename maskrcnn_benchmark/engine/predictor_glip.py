...
class GLIPDemo(object):
    def __init__(self,
        ...
        self.color = 255 ## initialize color
    ...
    def build_transform(self):
        ...
        if cfg.INPUT.TO_BGR255:# 已经转换成BGR的通道了，只需normalize到0-255
        ...
        transform = T.Compose(#预处理
            [
                T.ToPILImage(),#转换pil格式
    ...
    def build_tokenizer(self):#构建分词器
        cfg = self.cfg
        tokenizer = None
    ...
    def run_ner(self, caption):# Named Entity Recognition, NER
        noun_phrases = find_noun_phrases(caption)# 提取名词
        noun_phrases = [remove_punctuation(phrase) for phrase in noun_phrases]# 移除标点
        noun_phrases = [phrase for phrase in noun_phrases if phrase != '']#去除空字符
        ...
        for entity, label in zip(relevant_phrases, labels):
            try:
                ...
                for m in re.finditer(entity, caption.lower()):# 转换为小写查询，m包含起始位置
    ...
    def inference(self, original_image, original_caption):
        ...
        top_predictions = self._post_process_fixed_thresh(predictions)# 根据阈值筛选top结果
    ...
    def run_on_web_image(self, 
            ...
            custom_entity = None,
            alpha = 0.0):
        predictions = self.compute_prediction(original_image, original_caption, voc, custom_entity)
        print('predictions')
        print(predictions)
        print(predictions.extra_fields['scores'])
        top_predictions = self._post_process(predictions, thresh)
        print(top_predictions)
        result = original_image.copy() # result是预测结果图像，top_predictions是boxlist
    ...
    def compute_prediction(self, original_image, original_caption, voc, custom_entity = None):
        # image
        ...
        if isinstance(voc, list):
            ...
            caption_string_voc = ""
            tokens_positive_voc = []
            seperation_tokens_voc = " . "
            for word in voc:
                
                tokens_positive_voc.append([[len(caption_string_voc), len(caption_string_voc) + len(word)]])
                caption_string += word
                caption_string += seperation_tokens_voc
            
            tokenized_voc = self.tokenizer([caption_string_voc], return_tensors="pt")

            voc = caption_string_voc
            print(tokens_positive_voc)
        else:
            tokenized_voc = self.tokenizer([voc], return_tensors="pt")
            if custom_entity is None:# 没在列表中，使用分词器
                tokens_positive_voc = self.run_ner(voc)
            else:
                tokens_positive_voc = []
                for entity in custom_entity:
                    if "char_bounds" in entity:
                        # char bounds provided
                        tokens_positive_voc.append([list(entity["char_bounds"])])
                    else:
                        # online look for char bounds
                        for i, m in enumerate(re.finditer(entity["span"], voc)):
                            if i >= 1:
                                print(
                                    f"More than 1 match with the phrase {entity['span']} in {entity['span']}! We only take the first match."
                                )
                                break
                            tokens_positive_voc.append([[m.start(), m.end()]])
            print(tokens_positive_voc) 
        ...
        print(f"positive_map_label_to_token: {positive_map_label_to_token}")
        tic = timeit.time.perf_counter()

        # compute predictions
        with torch.no_grad():
            predictions = self.model(image_list, captions=[original_caption], voc=[voc], positive_map=positive_map_label_to_token)
    ...
    def _post_process(self, predictions, threshold=0.5):
        scores = predictions.get_field("scores") # 每个预测框box都有两个字段fields：scores，labels
        labels = predictions.get_field("labels").tolist()
        thresh = scores.clone() # 把阈值传给 scores 形状的列表
        for i, lb in enumerate(labels):
            if isinstance(self.confidence_threshold, float):
                thresh[i] = threshold
            elif len(self.confidence_threshold) == 1:
    ...
    def overlay_boxes(self, image, predictions, alpha=0.5, box_pixel = 3):
        ...
        for box, color in zip(boxes, colors):
            ...
            new_image = cv2.rectangle(
                ...
                new_image, tuple(top_left), tuple(bottom_right), (0,255,0), box_pixel)
    ...
    def overlay_entity_names(self, image, predictions, names=None, text_size=1.0, text_pixel=2, text_offset = 10, text_offset_original = 4):
        ...
        if self.entities and self.plus:
            for i in labels:
                if i <= len(self.entities):
                    ...
                    # new_labels.append('white blood celll')
                else:
        ...
        for box, score, label in zip(boxes, scores, new_labels):
            ...
            cv2.putText(
                ...
                image, s, (int(x), int(y)-text_offset_original), cv2.FONT_HERSHEY_SIMPLEX, text_size, (0,255,0), text_pixel, cv2.LINE_AA
            )
