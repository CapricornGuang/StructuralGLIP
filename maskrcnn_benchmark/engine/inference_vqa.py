...
def create_queries_and_maps_from_dataset(dataset, cfg):
    ...
    return all_queries, all_positive_map_label_to_token,labels, label_list
...
def create_queries_and_maps_modified(json_dict, paths, additional_labels = None, cfg = None):
...
def create_prompt(categories, cfg, tokenizer, model, img_paths):
    ...
    prompt_dict = {
        'color': f'What is the color of the flesh bump?',
        'shape': f'What is the shape of the flesh bump?',
        #'location': f'Where is this flesh bump located on?',
    }
    with open('/root/projects/vl/VL-model-medical-image-understanding/prompt_logs.txt', 'a+') as f:
        f.writelines(f'color: What is the color of the flesh bump? \n')
        f.writelines(f'shape: What is the shape of the flesh bump? \n')
        f.writelines(f'location: Where is this flesh bump located on?')
    ...
    for img_path in img_paths:
        ...
        color, shape, location = ans_dict['color'], ans_dict['shape'], None#ans_dict['location']
        caption = f'{shape} shape of flesh bump in the rectum'
        captions.append(caption)
    with open('/root/projects/vl/VL-model-medical-image-understanding/prompt_logs.txt', 'a+') as f:
        f.writelines('**************'*7+ '\n')
        f.writelines(f'Paths: {img_paths} \n')
        f.writelines(f'Captions: {captions}\n')

    return captions

def create_prompt_from_json(index, counter, json_dict, batch_size, paths,mode='hybrid'):
    #print('****', counter, len(paths))
    
    return [json_dict['prompts'][counter+i] for i in range(batch_size)], counter + len(paths)

import random

def reverse_random_words(text, percentage=0.0):
    # Split the text into words, keeping punctuation separate
    random.seed(42)
    words = []
    temp_word = ""
    for char in text:
    ...
    # Adjusting indices to exclude 'bump' and non-alphabetic characters
    valid_indices = [i for i, word in enumerate(words) if word.isalpha() and word != "bump"]
    
    # Determine number of words to reverse
    num_to_reverse = int(len(valid_indices) * percentage)
    
    # Randomly select indices from valid indices
    indices_to_reverse = random.sample(valid_indices, num_to_reverse)
    
    # Reverse selected words
    for index in indices_to_reverse:
        words[index] = words[index][::-1]
    
    # Rejoin words and punctuation into a single string
    return ''.join(words)
...
def generate_nonsense_word(length=5):
    # Generate a random word of given length using lowercase letters
    return ''.join(random.choice(string.ascii_lowercase) for _ in range(length))

def insert_random_nonsense_words(text, percentage=0.1, seed=42):
    ...
    # Determine number of words to insert
    num_to_insert = int(len(words) * percentage)

    # Positions where the nonsense words will be inserted
    positions = random.sample(range(len(words) + num_to_insert), num_to_insert)

    # Sort positions to insert words in the correct order
    positions.sort()

    # Insert nonsense words at the selected positions
    offset = 0
    for position in positions:
        # Generate a random nonsense word to insert
        nonsense_word = generate_nonsense_word()
        words.insert(position + offset, nonsense_word)
        offset += 1
...
def create_prompt_from_path_json(json_dict, labels, label_list, tokenizer, paths, mode='hybrid'): # ours with voc select
    captions = []
    names = []
    ...
    for i, path in enumerate(paths):
        ...
        for _index, label in enumerate(labels[0]): #class specific
            ...
            name = 'bump is pink circle in rectum'

            caption = 'bump is pink circle in rectum white yellow orange red brown pink pale tan gray-white gold cream ruby turquoise indigo violet \
            octagon circle round heart oblong oval small rounded jagged wide large bulbous spherical circular irregular diamond \
            smooth textured cracked striped shiny dull speckled raised rough granular grooved glossy veined pigmented uneven mottled interwoven lines patches complex reticular structure\
            rectal mucosal elevated demarcated creased folded isolated clustered solitary honeycombed' # gpt-blip-final
            
            start_i = len(objects_query)
            tokenized = tokenizer(name, return_tensors="pt")
        ...
        objects_query = caption

        assert objects_query == caption
    return captions, names, positive_maps
...
def filter_proposals_with_thresholds(boxlist, min_area=2000, max_area=300000, min_aspect_ratio=0.5, max_aspect_ratio=2.0):
    if len(boxlist) < 2:  # Check if BoxList is empty
        return boxlist
    bbox = boxlist.bbox
    width = bbox[:, 2] - bbox[:, 0]
    ...
    keep = (area >= min_area) & (area <= max_area) & (aspect_ratio >= min_aspect_ratio) & (aspect_ratio <= max_aspect_ratio)

    filtered_boxes = boxlist[keep]
    return filtered_boxes

def filter_proposals_by_score(boxlist, min_score=0.3):
    if len(boxlist) < 3:  # Check if BoxList is empty
        return boxlist
    """
    Filter out boxes with a score lower than `min_score`.

    Args:
        boxlist (BoxList): The BoxList object containing the proposals.
        min_score (float): The minimum score threshold for keeping a box.

    Returns:
        BoxList: A BoxList object with only the proposals that have a score >= `min_score`.
    """
...
def inference(
    ...
    tokeniszer_bert = AutoTokenizer.from_pretrained("MODEL/bert-base-uncased")
    ###########################################
    ...
    if task == "detection":
        ...
        all_queries, all_positive_map_label_to_token, labels, label_list = create_queries_and_maps_from_dataset(dataset, cfg) # todo
    elif task == "grounding":
        all_queries = [None]
            ...
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if task == "detection":
                        ...
                        voc, captions, positive_map_label_to_token = create_prompt_from_path_json(json_dict, labels, label_list, tokeniszer_bert, paths)
                    ...
                    output = model(images, captions=captions, voc=voc, positive_map=positive_map_label_to_token)
                    output = [o.to(cpu_device) for o in output]
